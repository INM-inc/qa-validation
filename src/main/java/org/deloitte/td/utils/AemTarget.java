...
public class AemTarget {
  ...
  public void run(String assetsPath, String rootPath) {
    ...
    for(MigrationDto m: dataMigration){
      ...
      File asset = this.getFile(assetsPath + "/" +m.getId());

      if (asset.exists() && !asset.isDirectory()) {
        String shaCode = generateSha(asset);
        foundAssets.add(m.getId()+","+asset.length()+","+m.getFileName()+","+shaCode+","+aemPath);
      } else {
        notFoundAssets.add(m.getId()+",Asset - Not Found,"+m.getFileName());
      }

      String assetFile = props.get("aemServer") + aemPath + "/" + m.getFileName();
      HttpURLConnection assetConn = assetAemValidation(assetFile,props.get("aemUserName"),props.get("aemPassword"));
